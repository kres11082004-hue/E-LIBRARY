...
import { Search, BookOpen, BookMarked, Filter } from "lucide-react";

const CATEGORIES = ["All", "Fiction", "Non-Fiction", "Science", "Technology", "History", "Philosophy", "Mathematics", "Literature", "Reference", "Thesis"];

function BookCard({ book, inMyList, onAdd }: { book: any; inMyList: boolean; onAdd: () => void }) {
  return (
    <div className="bg-card border rounded-xl overflow-hidden hover:shadow-md transition-all group">
      <Link href={`/books/${book.id}`}>
        <div className="h-44 bg-muted flex items-center justify-center overflow-hidden cursor-pointer">
          {book.coverUrl ? (
      ...
      <div className="p-4">
        <Link href={`/books/${book.id}`}>
          <h3 className="font-semibold text-sm text-foreground line-clamp-2 leading-snug hover:text-primary cursor-pointer">{book.title}</h3>
        </Link>
        ...
        {book.isAvailablePhysical && (
          <p className="text-xs text-green-600 mt-2 font-medium">
            Physical: {book.availableCopies}/{book.totalCopies} available
          </p>
        )}
      </div>
    </div>
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      <div>
        ...
        <p className="text-muted-foreground text-sm mt-0.5">{books.length} books available</p>
      </div>
